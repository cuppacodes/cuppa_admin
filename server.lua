...
RegisterCommand(config.prefix .. ' kick', function(source, args)
...
RegisterCommand(config.prefix .. ' ban', function(source, args)
    ...
    -- Ban message shown to player
    local banMsg = 'Banned: ' .. reason
    if duration then
        banMsg = banMsg .. ' (expires in ' .. durationText .. ')'
    end

    -- Insert into bans table and capture the ban ID
    MySQL.Async.insert('INSERT INTO bans (name, license, discord, ip, reason, expire, bannedby) VALUES (?, ?, ?, ?, ?, ?, ?)', {
        ...
        if banId then
            -- Log all identifiers to console
...
RegisterCommand(config.prefix .. ' unban', function(source, args)
...
RegisterCommand(config.prefix .. ' heal', function(source, args)
...
RegisterCommand(config.prefix .. ' kill', function(source, args)
...
RegisterCommand(config.prefix .. ' revive', function(source, args)
...
RegisterCommand(config.prefix .. ' freeze', function(source, args)
...
RegisterCommand(config.prefix .. ' unfreeze', function(source, args)
...
RegisterCommand(config.prefix .. ' goto', function(source, args)
...
RegisterCommand(config.prefix .. ' bring', function(source, args)
...
RegisterCommand(config.prefix .. ' vehicle', function(source, args)
    if not hasPermission(source, 'vehicle') then return notify(source, 'No permission', 'error') end
    local model = args[1]
    if not model then return notify(source, 'Usage: cc vehicle <model>', 'error') end
...
RegisterCommand(config.prefix .. ' fix', function(source, args)
    if not hasPermission(source, 'fix') then return notify(source, 'No permission', 'error') end
    local target = getTarget(source, args[1]) or source
    if target == 0 then return notify(source, 'Console requires a player ID', 'error') end
    if not exports.qbx_core:GetPlayer(target) then return notify(source, 'Player not found', 'error') end
...
RegisterCommand(config.prefix .. ' dv', function(source, args)
...
RegisterCommand(config.prefix .. ' giveitem', function(source, args)
...
RegisterCommand(config.prefix .. ' setjob', function(source, args)
...
RegisterCommand(config.prefix .. ' setgang', function(source, args)
...
RegisterCommand(config.prefix .. ' givecash', function(source, args)
...
RegisterCommand(config.prefix .. ' givebank', function(source, args)
...
RegisterCommand(config.prefix .. ' armor', function(source, args)
...
RegisterCommand(config.prefix .. ' setmodel', function(source, args)
...
RegisterCommand(config.prefix .. ' noclip', function(source, args)
    if not hasPermission(source, 'noclip') then return notify(source, 'No permission', 'error') end
    local target = getTarget(source, args[1]) or (source == 0 and nil or source)
